...
def match_edges_chunk(args):
    chunk, title_to_folder, folder_to_id, all_titles = args
    local_edges = []
    for citing_title, cited_titles in chunk:
        ...
        for ref_title in cited_titles:
            match = process.extractOne(ref_title, all_titles, scorer=fuzz.token_sort_ratio, score_cutoff=80)
            if match:
                matched_title = match[0]
                cited_folder = title_to_folder[matched_title]
                cited_id = folder_to_id[cited_folder]
                if citing_id != cited_id:
                    local_edges.append((citing_id, cited_id))
    return local_edges
...
def build_graph(folder_to_title, title_to_folder, paper_refs):
    ...
    refs_items = list(paper_refs.items())
    n_chunks = os.cpu_count()
    chunk_size = (len(refs_items) + n_chunks - 1) // n_chunks
    chunks = [refs_items[i:i+chunk_size] for i in range(0, len(refs_items), chunk_size)]

    args = [(chunk, title_to_folder, folder_to_id, all_titles) for chunk in chunks]

    all_edges = []
    with ProcessPoolExecutor(max_workers=n_chunks) as executor:
        for result in executor.map(match_edges_chunk, args):
...
def save_graph_and_metadata(graph, path_prefix="citation_graph"):
    os.makedirs("new_build_parallel", exist_ok=True)
    with open(f"new_build_parallel/{path_prefix}.gpickle", "wb") as f:
        pickle.dump(graph, f)
    with open(f"new_build_parallel/{path_prefix}_folder_to_id.json", "w") as f:
        json.dump(folder_to_id, f)
    with open(f"new_build_parallel/{path_prefix}_id_to_folder.json", "w") as f:
        json.dump(id_to_folder, f)
    debug(f"Saved graph and metadata with prefix 'new_build_parallel/{path_prefix}'")
