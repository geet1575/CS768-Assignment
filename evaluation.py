...
def load_dataset(dataset_root):
    ...
    debug("Scanning dataset...")

    for paper_folder in os.listdir(dataset_root):
        paper_path = os.path.join(dataset_root, paper_folder)
        if os.path.isdir(paper_path):
            title = extract_title(paper_path)
            if title:
                paper_titles[title] = paper_folder
                citations = extract_citations(paper_path)
                paper_refs[title] = citations
                debug(f"Parsed {paper_folder} → title: {title[:60]}...")

    return paper_titles, paper_refs
...
def build_graph(paper_titles, paper_refs):
    global folder_to_id, id_to_folder, NUM_EDGES

    graph = nx.DiGraph()
    all_titles = list(paper_titles.keys())

    debug("Building citation graph...")

    folder_list = list(paper_titles.values())
    ...
    for citing_title, cited_titles in paper_refs.items():
        if citing_title not in paper_titles:
            continue
        citing_folder = paper_titles[citing_title]
        citing_id = folder_to_id[citing_folder]

        for ref_title in cited_titles:
            match = get_close_matches(ref_title, all_titles, n=1, cutoff=0.8)
            if match:
                matched_title = match[0]
                cited_folder = paper_titles[matched_title]
...
def save_graph_and_metadata(graph, path_prefix="citation_graph"):
    ...
    with open(f"{path_prefix}_folder_to_id.json", "w") as f:
        json.dump(folder_to_id, f)
    with open(f"{path_prefix}_id_to_folder.json", "w") as f:
        json.dump(id_to_folder, f)
    debug(f"Saved graph and metadata with prefix '{path_prefix}'")

def load_graph_and_metadata(path_prefix="citation_graph"):
    graph = nx.read_gpickle(f"{path_prefix}.gpickle")
    with open(f"{path_prefix}_folder_to_id.json", "r") as f:
        folder_to_id = json.load(f)
    with open(f"{path_prefix}_id_to_folder.json", "r") as f:
...
def main():
    ...
    paper_titles, paper_refs = load_dataset(dataset_root)
    global graph
    graph = build_graph(paper_titles, paper_refs)
    ...
    # Debug example: show outgoing edges of node 0
    if DEBUG:
